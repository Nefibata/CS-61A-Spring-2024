...
def about(subject):
    ...
    def reF(string):
        ...
        string4 = re.sub(r'[^\w\s]', '', string_low)
        string_list=string4.split(' ')
        for s in subject :
            if s in string_list :
                return True
        return False
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    if len(typed_words)==0 or len(typed_words)==0 :
        if typed==source:
            return 100.0
        return 0.0
    else:
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list :
        return typed_word
    for w in word_list :
        diff_num=diff_function(typed_word,w,limit)
        if diff_num<=limit :
            if not diff_num in  result.keys():
                result[diff_num]=w
    if len(result)==0:
        return typed_word
    else :
        re=result[min(n for n in result.keys())]
        return re
...
def feline_fixes(typed, source, limit):
    ...
    """
    count=0
    abs_len=abs(len(typed)-len(source))
    if abs_len>limit:
        return limit +1
    for i in range(min(len(typed),len(source))):
        if typed[i]!=source[i]:
            ...
            if count>limit:
                return limit+1
    return count + abs_len
    """
    def inF (typed_1,source_1,count):
        if limit<count:
            return limit+1 
        if len(typed_1)==0 and len(source_1)==0:
            return count
        if len(typed_1)==0 or len(source_1)==0:
            return inF("","",count+max(len(source_1),len(typed_1)))
        if typed_1[0]==source_1[0]:
            return inF(typed_1[1:],source_1[1:],count)
        else:
            return inF(typed_1[1:],source_1[1:],count+1)
    return inF(typed,source,0)
    # END PROBLEM 6
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    re=[[] for _x in player_indices]
    for i in word_indices:
        word=get_word(match,i)
        min_play=0
        min_tim=time(match,0,i)
        for j in player_indices:
            if time(match,j,i)<min_tim:
                min_play=j
                min_tim=time(match,j,i)
        re[min_play].append(word)
    return re
